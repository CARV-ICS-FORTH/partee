...
#define tsc()                                     \
	({ unsigned long long x; rdtscll(x); x; } \
	)
#define rdpmcll(counter, x)               \
	__asm__ __volatile__ ( "rdpmc"    \
			       ...
			       : "c" (counter) )
#define pmc0()                                       \
	({ unsigned long long x; rdpmcll(0, x); x; } \
	)
#define pmc1()                                       \
	({ unsigned long long x; rdpmcll(1, x); x; } \
	)

double proc_frequency(void)
{
	return 550e6;
}

#endif /* ifdef CONFIG_X86_TSC */
#endif /* if defined (i386) && defined (linux) */
...
static void print_memory_usage(void)
{
	...
	fprintf( stderr, "blocks alloced: %lu, %.1f%% 8K\n",
	         ...
	         (100.0 * total_8kblocks) /
	         (total_8kblocks + total_otherblocks) );
...
static void record_alloc(unsigned long nbytes)
{
	...
	for (i = 0; i < sizerec_n; i++)
		if (allocs[i].line == __rc_line &&
		    allocs[i].file == __rc_file) {
