 ...
 */
struct page_state {
	INT_PTR              first_page; /* is this the first page of the
	                                  * region? */
	...
	struct page     *old_address; /* the address of the page before
	                               * serialization */
};
struct page_list {
	struct page      *pg;
...
struct page_list* make_page_list(struct page *p, region temp)
{
	...
	for ( ; p != NULL; p = p->next) {
		n =
		        (struct page_list*)rstralloc( temp,
 ...
 */
void serialize_pages(struct page_list *pgs, int data, int state,
                     INT_PTR first_page, INT_PTR last_page,
                     enum allocator_kinds akind, enum page_kinds pkind,
                     int size, struct allocator *a)
{
	...
	for ( ; pgs != NULL; pgs = pgs->next) {
		...
		first_page     = 0; /* only the first page in the list can
		                     * possibly be the first page of the region
		                     * */
		ps.last_page   = (pgs->next == NULL) && last_page;
		ps.akind       = akind;
		ps.pkind       = pkind;
		ps.size        = size *
		                 ( (pkind == BIG) ? pgs->pg->pagecount : 1 );
		...
		fprintf( stderr,
		         ...
		         (INT_PTR)ps.old_address, ps.size,
		         ( (INT_PTR)pgs->pg->available ) -
		         ( (INT_PTR)pgs->pg ) );
#endif /* ifndef NMEMDEBUG */
		numbytes = write(data, pgs->pg, ps.size);
		if (numbytes != ps.size) {
			fprintf(stderr,
			        "Serialization failed: Could not write full page.\n");
			exit(1);
		}
		numbytes = write( state, &ps, sizeof(struct page_state) );
		if ( numbytes != sizeof(struct page_state) ) {
			fprintf(stderr,
			        "Serialization failed: Could not write page state information.\n");
			exit(1);
		}
	}
#ifndef NMEMDEBUG
	if (total != 0)
		fprintf(stderr,
		        "\tPage list of size %ld bytes uses %ld bytes = %0.2f\n",
...
#ifndef NMEMDEBUG
	fprintf(stderr,
	        "Region: serializing pages (normal pages first, then big pages)\n");
#endif /* ifndef NMEMDEBUG */

	serialize_pages( normal, data, state, 1, (big_normal == NULL), NORMAL,
	                 REGULAR, RPAGESIZE, &(r->normal) );
	serialize_pages(big_normal, data, state, 0, (big_normal != NULL),
	                NORMAL, BIG, RPAGESIZE, NULL);
...
{
	int data  = open(datafile, O_WRONLY | O_CREAT | O_TRUNC,
	                 S_IREAD | S_IWRITE);
	int state = open(statefile, O_WRONLY | O_CREAT | O_TRUNC,
	                 S_IREAD | S_IWRITE);
 ...
 *  requires that pages be aligned at addresses where the last RPAGELOG bits are
 * 0's.
 */
...
#ifndef NMEMDEBUG
	if ( old_address &&
	     (*( map->map + ( ( (INT_PTR)old_address ) >> RPAGELOG ) ) == 0) )
		fprintf(stderr,
		        "Warning: The pointer %lx has no translation.\n",
		        (long)(INT_PTR)old_address);
#endif /* ifndef NMEMDEBUG */
	return ( *( map->map + ( ( (INT_PTR)old_address ) >> RPAGELOG ) ) ) +
	       ( ( (INT_PTR)old_address ) &
	         ( ( (INT_PTR)0xFFFFFFFF ) >> (32 - RPAGELOG) ) );
}
...
void allocate_regions(int state, translation map)
{
	...
	for ( ; ; ) {
		...
		if ( numbytes != sizeof(struct page_state) ) {
			fprintf(stderr,
			        "Error: Could not populate page map.\n");
			exit(1);
		}
		 ...
		 *
		 *  If the page is the first page in a new region, it will have
		 * a region object.
		 *  Rather than allocate a new page, we allocate a new region in
		 * this case (which indirectly allocates the page).
		 *  If the page is not the first page in a new region, we
		 * allocate it and attach it to the current region.
		 *  The logic is slightly different depending on whether the
		 * page is normal or atomic.
		 */
		if (ps.first_page) {
			r         = __newregion();
		...
		/* Now we record the address of the new page(s) as the
		 * translation of the address of the old page(s). */
		if ( ( ( ( (INT_PTR)ps.old_address ) >> RPAGELOG ) <<
		       RPAGELOG ) != (INT_PTR)ps.old_address ) {
...
/*
 *  Update all the pointers on a page to reflect the new locations after
 * deserialization.
 *  The update function is type-dependent and supplied by the user.
 *
 *  The alignment to RALIGNMENT must be consistent with the way the allocator
 * lays out data in the first
 *  place.  This implementation is a bit simpler than what the region allocator
 * does and may not work
 *  for arrays if RALIGNMENT is anything other than a word boundary (4).
 *
 *  Also a bit of a hack here is that we get the size of the object being
 * scanned from the update function,
 *  but in reality we need to know the size before we perform the update in case
 * the object would extend
 *  past the end of the page (in which case it is not in fact an object to be
 * updated).  However, every page
 *  must be large enough to hold at least its first object, so this
 * implementation is in fact correct.
 *
 */
...
/*
 *  Deserializing page data is the most complex operation.  A page is a block of
 * memory.  Serialization/deserialization
 *  relies on a page having only one type of user data on it, but there is
 * allocator management information stored on a page
 *  which must also be updated and not overwritten by accident.  In particular,
 * the start of the page always contains
 *  a "struct page" object with all of the vital statistics of the page.  Note
 * that the last field of a page struct can
 *  (and is) used for user data, even though it has a name "previous" that
 * suggests something else.  In fact this field is
 *  only used for system bookkeeping when the page is on the free list.
 *
 *  If this page is the first page in a region, it will also have a region
 * object immediately after the page object.
 *
 *  All of the pointer manipulation herein arises from the need to have a mix of
 * information from the new page/region objects
 *  and the serialized page/region objects wind up in the final page/region
 * objects.
 */
...
void deserialize_pages(int data, int state, translation map, Updater *update)
{
	...
	while (numbytes != 0) {
		if ( numbytes != sizeof(struct page_state) ) {
			fprintf( stderr,
			         "Error: Could not read page state. Bytes read = %d; bytes desired = %lu.\n", numbytes,
			         (long unsigned int)sizeof(struct page_state) );
			exit(1);
		}
		newp = (struct page*)translate_pointer(map, ps.old_address);
		mem  = ( (char*)newp ) + offsetof(struct page, previous); /*
		                                                           * compute
		                                                           * the
		                                                           * first
		                                                           * location
		...
		/* save structure of the new page that should not be obliterated
		 * by reading the old page */
		p               = *newp; /* save the page structure */
		save_new_region = *( (region)mem ); /* save the region
		...
		fprintf( stderr,
		         ...
		         (INT_PTR)ps.old_address, (INT_PTR)newp, ps.size,
		         ( (INT_PTR)newp->available ) -
		         ( (INT_PTR)ps.old_address ) );
#endif /* ifndef NMEMDEBUG */

		/*
		 *  At this point we have overwritten the newly allocated page
		 * header with the old (serialized) header of the page.
		 *  While neither the old nor new headers are completely correct
		 * at this point, the new header requires fewer
		 *  pointer updates than the old header.  So, here we overwrite
		 * the old header with the new header again.  There are
		 *  two wrinkles: The very last field of a page header is used
		 * only when the page is on the free list and is in fact
		 *  the start of allocation when the page is in use.  So, we
		 * copy and restore that field from the serialized version
		 *  so that we don't lose any data.  Also, we actually want the
		 * available field from the old page; we save it and translate
		 * it
		...
		*newp           = p; /* restore the current page structure */
		...
		/* If this page has a region object, restore the region and
		 * update its allocation state. */
		if (ps.first_page) {
			/* The following line must be consistent with where the
			 * region is originally layed out in newsubregion(). */
			region r = (region)mem;

			save_old_region = *r;
			*r              = save_new_region;
			/* update the allocation state of the region */
			update_allocator( map, &(r->normal),
			                  &(save_old_region.normal) );
			/*      update_allocator(map, &(r->atomic),
			 * &(save_old_region.atomic)); */
		 ...
		 *  If this is the last page in the region, move to the next
		 * update function for the next region.
		 */
		if (update != NULL) {
			/* update_page(mem, ((char *) newp) + ps.actual_size,
			 * map, *update); */
...
{
	struct translation_ *t =
	        (struct translation_*)rstralloc( r,
	                                         sizeof(struct translation_) );

	t->reg = __newregion();
	/* We rely on this map be zeroed out by the memory allocator. */
	t->map =
	        (void**)rstralloc( t->reg,
	                           sizeof(void*) * ( 1 << (32 - RPAGELOG) ) );
	return t;
}
...
/*
 *  Deserialize a set of regions.  "update" must be an array of update
 * functions, one for
 *  each region to be read, in the same order as the regions appear in the data
 * file.  There
 *  is no need to NULL-terminate this array, as the processing stops with the
 * last region in the
 *  file.  However, disaster may strike if the update function array has a
 * length other than
 *  the number of regions in the file or the functions appear in the incorrect
 * order.
 */
translation deserialize(char *datafile, char *statefile, Updater *update,
                        region r)
{
	translation t     = new_translation(r);
	...
	if ( RPAGESIZE != (1 << RPAGELOG) )
		fprintf(stderr,
		        "Error: Unexpected page size in deserialization\n");
